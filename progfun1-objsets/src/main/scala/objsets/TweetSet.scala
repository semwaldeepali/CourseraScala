...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    throw java.util.NoSuchElementException("Empty set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    if p(elem) then
      left.filterAcc(p, acc).union(right.filterAcc(p, acc)).incl(elem) // incl element after union or before sending to union ?
    else
  ...
  def mostRetweeted: Tweet =
    ...
    //both non empty
    else if !left.isEmpty && !right.isEmpty then
      val lTweet = left.mostRetweeted
      val rTweet = right.mostRetweeted
      if lTweet.retweets > rTweet.retweets then
        if lTweet.retweets > elem.retweets then lTweet
        else elem
      else
        if rTweet.retweets > elem.retweets then rTweet
        else elem
    //right empty
  ...
  override def descendingByRetweet: TweetList =
    ...
    Cons(twt, remove(twt).descendingByRetweet)

...
import patmat.Huffman.{convert, createCodeTree, decode, encode}
...
trait Huffman extends HuffmanInterface:
  ...
  def weight(tree: CodeTree): Int =  tree match
    case Fork(l, r, cList, w) => w
    case Leaf(c, w) => w

  def chars(tree: CodeTree): List[Char] = tree match
    case Fork(l, r, cList, w) => cList
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    def insert(c: Char, acc: List[(Char, Int)]) : List[(Char, Int)] =
      if acc.isEmpty then List((c,1))
      else
    ...
    def visit(chars: List[Char], acc: List[(Char, Int)]) : List[(Char, Int)] =
      if chars.isEmpty then acc
      else visit(chars.tail, insert(chars.head, acc))
  ...
  def createCodeTree(chars: List[Char]): CodeTree =
    ...
    val leafList = makeOrderedLeafList(freqTable)
    val tree = until(singleton, combine)(leafList)
    tree match
      case x :: Nil => x
      case node1 :: node2 :: Nil => makeCodeTree(node1,node2)
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    def iter(t: CodeTree, bits:List[Bit], acc: List[Char]) : List[Char] =
      if bits.isEmpty then
        ...
        t match
          ...
          case Fork(left, right, chars, weight) =>
            if bits.head == 1 then iter(right,bits.tail, acc)
            else iter(left, bits.tail, acc)
    iter(tree, bits, List())
  ...
  def getCode(tree: CodeTree, char: Char): List[Bit] =
    def iter(t: CodeTree, acc: List[Bit]): List[Bit] = t match
      case Fork(left, right, chars, weight) => iter(left, acc ::: List(0)) ::: iter(right, acc ::: List(1))
      case Leaf(c, weight) =>
        if c == char then acc
        else List()
    iter(tree, List())
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    if table.isEmpty then throw java.util.NoSuchElementException("character not in table")
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable =
    def iter(a: CodeTable, b: CodeTable, acc : CodeTable) : CodeTable =
      if a.isEmpty then acc:::b
      else iter(a.tail, b, acc:::List(a.head))
    iter(a,b,List())
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] =
    ...
    def iter(txt: List[Char], acc:List[Bit]) : List[Bit] =
      if txt.isEmpty then acc
      else iter(txt.tail, acc:::codeBits(table)(txt.head))
...
object Main extends App:
  ...
  println(convert(t1))
  val txt = List('h', 'e', 'l', 'l', 'o', ',', ' ', 'w', 'o', 'r', 'l', 'd')
  println(createCodeTree(txt))
